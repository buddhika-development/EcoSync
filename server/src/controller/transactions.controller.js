...
export const _new_transaction = async (req, res) => {
    ...
    if(user_id === null || transaction_amount === null) {
        return sendResponse(res, ERROR("Missing required fields: user_id and transaction_amount"))
    }

    const transaction_response = await _insert_transaction(user_id, transaction_amount)

    if(transaction_response.error) {
        return sendResponse(res, ERROR(transaction_response.message, transaction_response.error.details))
    }

    return sendResponse(res, SUCCESS(transaction_response.message, transaction_response.data))
}
